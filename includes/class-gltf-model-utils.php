...
class Gltf_Model_Utils {
	static function enqueue_common_gltf_scripts( $version ) {
		...
		wp_enqueue_script( 'vrcontrols', plugin_dir_url( dirname( __FILE__ ) ) . 'js/VRControls.js', array( 'threejs' ), $version, false );
		wp_enqueue_script( 'vreffect', plugin_dir_url( dirname( __FILE__ ) ) . 'js/effects/VREffect.js', array( 'threejs' ), $version, false );
	}
